...
$(function(){
	$('#infoList').datagrid({
		...
		url:"<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=getInfoList4Page&sort=create_time&order=desc",
		columns:[[
					{
						field : 'id',
						title : 'id',
...
function disconn(){
    ...
    if(sels==""||sels==null){
    	...
    	param.id=sels.id;
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=closeDb";
...
function reconn(){
    ...
    if(sels==""||sels==null){
    	...
    	var param=new Object();
    	param.id=sels.id;
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=restartDb";
		$.post(url,param,function(data,stats){
			if(stats=="success" ){
				$.messager.show({
					msg : "操作成功",
	...
	function addOne(){
		...
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=createInfo";
	    ...
	    if(sels==""||sels==null){
	    	...
	    	$.messager.prompt('提示框', '输入密码:', function(r) {
	    	    if( r ) {
	    	    	...
	    	    	param.id=sels.id;
	    	    	param.db_password=r;
	    			var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=modifyPass";
	    			$.post(url, param, function(data, stats) {
	    				if (stats == "success" ) {
	    					$.messager.show({
	    						msg : "操作成功",
	...
	function updateOne(){
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=updateInfo";
	...
	function remove(){
		...
		if(sels == ''|| sels==null){
			...
			if(result == true){
				var querydata = $('#searchForm').serializeObject();
				var id = sels.id;
				var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=MicroDbList&groovySubName=delInfo";
				url=url+"&id="+id;
