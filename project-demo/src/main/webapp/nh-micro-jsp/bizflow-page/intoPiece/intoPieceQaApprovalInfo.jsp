...
function submit_approval(){
	...
	dataO.id="<%=id%>";
 	...
 	var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=bizflow_intopiece_qa_approval&groovySubName=submitApproval";
...
<button onclick="submit_approval()">提交进件质检审批意见</button>
</c:if>
...
<c:if test="${formdata.dbcol_ext_bizflow_intopiece_qa_approval_status=='yes'}">checked="true"</c:if>
>同意</input>
&nbsp;
<input type="radio" name="approval_status" value="no" 
<c:if test="${formdata.dbcol_ext_bizflow_intopiece_qa_approval_status=='no'}">checked="true"</c:if>
>不同意</input>
&nbsp;
<input type="radio" name="approval_status" value="back" 
<c:if test="${formdata.dbcol_ext_bizflow_intopiece_qa_approval_status=='back'}">checked="true"</c:if>
>打回</input>
</td>
</tr>
<tr>
<td><textArea name="approval_text">${formdata.dbcol_ext_bizflow_intopiece_qa_approval_text}</textArea></td>
</tr>
</table>
</form>
