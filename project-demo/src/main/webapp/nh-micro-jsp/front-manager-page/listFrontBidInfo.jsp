...
$(function(){
	$('#infoList').datagrid({
		...
		url:"<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=front_bid_list&groovySubName=getInfoList4Page",
		columns:[[
					{
						field : 'id',
						title : 'id',
	...
	var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=front_bid_list&groovySubName=toEditBidGo&operFlag=create";
	...
	function update(){
		...
		if(sels == ''|| sels==null){
			...
			var id = sels.id;
		var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=front_bid_list&groovySubName=toEditBidGo&operFlag=update&id="+id;
		window.location.href=url;
		}
	}
	/*删除*/
	function remove(){
		...
		if(sels == ''|| sels==null){
			...
			if(result == true){
				var querydata = $('#searchForm').serializeObject();
				var id = sels.id;
				var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=front_bid_list&groovySubName=delInfo";
				url=url+"&id="+id;
	...
	<div id="infoQuery" class="dQueryMod" region="north"
		...
		<form id="searchForm">
			<table id="searchTable">
				<tr>
					<td>理财标的编码：</td>
					<td><input type="text" id="bid_code" name="bid_code" /></td>					
